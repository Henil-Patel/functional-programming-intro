...
trait Huffman extends HuffmanInterface:
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = 
    def iter(active_tree: CodeTree, bits: List[Bit]): List[Char] = {
      active_tree match {
        case Fork(l, r, _, _) => iter(if (bits.head == 0) l else r, bits.tail)
        case Leaf(c, _) => 
          if(bits.isEmpty) List(c)
          else c :: iter(tree, bits) 
      }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = 
    def iter(active_tree: CodeTree, active_char: Char): List[Bit] = {
      active_tree match {
        case Leaf(c, w) => List()
        case Fork(l, r, c, w) => 
          ...
          // if (chars(r).contains(active_char))
          else 
          {
    ...
    if (!text.tail.isEmpty) iter(tree, text.head) ::: encode(tree)(text.tail)
    else iter(tree, text.head)
